...
class Model:
    ...
    async def run(self,data:ModelInput)->str:
        ...
        while(current_try<=max_try):
            ...
            response = await workflow.execute_activity(execute_base_model,args=[data.base_model,chat_history],start_to_close_timeout=timedelta(seconds=300))
            
            formatted_judge_input = f"{data.judge_model.model_prompt}\n\n Original Prompt given to AI :\n {data.base_model.model_prompt}\n\n AI Output to Grade:\n{response}"
            result = await workflow.execute_activity(execute_judge_model,args=[data.judge_model,formatted_judge_input],start_to_close_timeout=timedelta(seconds=300))
